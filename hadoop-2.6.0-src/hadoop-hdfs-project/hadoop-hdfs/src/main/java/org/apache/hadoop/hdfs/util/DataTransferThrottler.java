...
public class DataTransferThrottler {
  private final long period;          // period over which bw is imposed 周期,单位毫秒
  private final long periodExtension; // Max period over which bw accumulates.
  private long bytesPerPeriod;  // total number of bytes can be sent in each period 每个周期内,允许的带宽量
  private long curPeriodStart;  // current period starting time 周期的开始时间
  private long curReserve;      // remaining bytes can be sent in the period 当前该周期内还能发送的带宽量
  private long bytesAlreadyUsed;//目前已经使用的带宽量
  ...
  public DataTransferThrottler(long period, long bandwidthPerSec) {
    ...
    this.period = period;
    this.curReserve = this.bytesPerPeriod = bandwidthPerSec*period/1000;//bandwidthPerSec / 1000 表示每个毫秒允许多少带宽,period表示周期,即该周期内允许的带宽数量,单位为毫秒
  ...
  public synchronized void throttle(long numOfBytes, Canceler canceler) {
    ...
    while (curReserve <= 0) {
      ...
      long curPeriodEnd = curPeriodStart + period;//该周期的结束时间

      if ( now < curPeriodEnd ) {//说明该周期内不允许继续使用带宽
        // Wait for next period so that curReserve can be increased.
        try {
          wait( curPeriodEnd - now );//则睡眠一阵子
