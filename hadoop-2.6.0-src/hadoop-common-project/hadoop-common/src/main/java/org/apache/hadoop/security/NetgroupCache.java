 ...
 * 参考JniBasedUnixGroupsNetgroupMapping
 * JniBasedUnixGroupsNetgroupMapping类使用了本类作为缓存
 */
@InterfaceAudience.LimitedPrivate({"HDFS", "MapReduce"})
@InterfaceStability.Unstable
public class NetgroupCache {
  private static boolean netgroupToUsersMapUpdated = true;//true表示group对user集合的映射有变化
  //key是group,value是该gourp对应的user集合
  private static Map<String, Set<String>> netgroupToUsersMap = new ConcurrentHashMap<String, Set<String>>();

  //key是user,value是该user对应的group集合
  ...
  public static void getNetgroups(final String user,List<String> groups) {
    if(netgroupToUsersMapUpdated) {//当映射有变化的时候执行
      netgroupToUsersMapUpdated = false; // at the beginning to avoid race
      //update userToNetgroupsMap 循环每一个组
      for(String netgroup : netgroupToUsersMap.keySet()) {
        for(String netuser : netgroupToUsersMap.get(netgroup)) {//该组对应的user循环
          // add to userToNetgroupsMap 为该user添加该组
