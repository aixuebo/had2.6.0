...
public abstract class ChecksumFs extends FilterFs {
  ...
  private boolean verifyChecksum = true;//是否校验
  ...
  /** Return the name of the checksum file associated with a file.获取校验文件路径*/
  public Path getChecksumFile(Path file) {
    return new Path(file.getParent(), "." + file.getName() + ".crc");
  }

  /** Return true iff file is a checksum file name.该file是否是校验和文件*/
  ...
  private int getSumBufferSize(int bytesPerSum, int bufferSize)
    throws IOException {
    int defaultBufferSize =  getMyFs().getServerDefaults().getFileBufferSize();//文件的缓存大小
