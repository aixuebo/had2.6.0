...
public class DelegationTokenRenewer
  ...
  public static class RenewAction<T extends FileSystem & Renewable>
    ...
    private final WeakReference<T> weakFs;//文件系统的弱引用
    private Token<?> token; 
    ...
    private RenewAction(final T fs) {
      ...
      updateRenewalTime(renewCycle);//更新时间
    }
  ...
  private static final long RENEW_CYCLE = 24 * 60 * 60 * 1000;//24h ,更新周期

  @InterfaceAudience.Private
  @VisibleForTesting
  public static long renewCycle = RENEW_CYCLE;

  /** Queue to maintain the RenewActions to be processed by the {@link #run()} 队列*/
  ...
  /** Add a renew action to the queue. 添加一个文件系统到队列中*/
