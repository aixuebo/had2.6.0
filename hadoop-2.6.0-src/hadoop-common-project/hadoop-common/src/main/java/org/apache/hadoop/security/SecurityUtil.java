...
public class SecurityUtil {
  ...
  // by the user
  @VisibleForTesting
  static boolean useIpForTokenService;//是否使用ip做安全访问
  @VisibleForTesting
  ...
  public static void setTokenServiceUseIp(boolean flag) {
    ...
    hostResolver = !useIpForTokenService
        ? new QualifiedHostResolver()//不使用ip做安全
        : new StandardHostResolver();//是使用ip做安全
  }
  ...
  /**
   * components[0] / host / @components[2]
   */
  ...
  static class StandardHostResolver implements HostResolver {
    //返回Sc-201409041654/192.168.10.203,如果是ip为host,则结果为/192.168.10.203
    @Override
  ...
  protected static class QualifiedHostResolver implements HostResolver {
    ...
    public InetAddress getByName(String host) throws UnknownHostException {
      ...
      if (IPAddressUtil.isIPv4LiteralAddress(host)) {
        ...
        addr = InetAddress.getByAddress(host, ip);//192.168.10.203/192.168.10.203
      } else if (IPAddressUtil.isIPv6LiteralAddress(host)) {
        // use ipv6 address as-is
        byte[] ip = IPAddressUtil.textToNumericFormatV6(host);
        addr = InetAddress.getByAddress(host, ip);//192.168.10.203/192.168.10.203,注意ip为6位的
    ...
    InetAddress getByNameWithSearch(String host) {
      ...
      if (host.endsWith(".")) { // already qualified?
        ...
        for (String domain : searchDomains) {
          ...
          addr = getByExactName(host + dot + domain);//host.domain是否找到对应的ip
          if (addr != null) break;
        }
      }
      return addr;
