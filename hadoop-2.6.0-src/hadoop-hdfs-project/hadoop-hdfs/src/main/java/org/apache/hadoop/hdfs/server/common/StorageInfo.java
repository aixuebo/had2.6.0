 ...
 * 一个节点对应一个该对象,不管节点是namenode还是datanoed
 */
...
public class StorageInfo {
  ...
  protected final NodeType storageType; // Type of the node using this storage 节点类型namenode还是datanoed
  ...
  /** Validate and set storage type from {@link Properties} 检查节点类型与配置文件中的类型是否一致,不一致则抛异常*/
  ...
  /** Validate and set ctime from {@link Properties}  设置ctime*/
  ...
  protected void setNamespaceID(Properties props, StorageDirectory sd)
      throws InconsistentFSStateException {
    int nsId = Integer.parseInt(getProperty(props, sd, "namespaceID"));//获取namespaceID值
