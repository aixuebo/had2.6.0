...
public class FileUnderConstructionFeature implements INode.Feature {
   ...
   * @param INodeFile 待更新文件,即该文件的最后一个数据块长度要被更新
   * @param lastBlockLength 最后更新的数据长度
   ...
   * and its size is 0.
   * 该参数的文件的最后一个数据块是正在构建的数据块,并且该数据块字节长度为0,则将该数据块从文件中删除引用，并且将数据库添加到参数集合中
   */
  void cleanZeroSizeBlock(final INodeFile f,final BlocksMapUpdateInfo collectedBlocks) {
    ...
    if (blocks != null && blocks.length > 0
      ...
      //
      BlockInfoUnderConstruction lastUC = (BlockInfoUnderConstruction) blocks[blocks.length - 1];
