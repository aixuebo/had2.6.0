...
public class INodeFile extends INodeWithAdditionalFields
  ...
  /** 
   * Bit format: 8个字节,一个long表示很多含义
   * [4-bit storagePolicyID][12-bit replication][48-bit preferredBlockSize]
   */
  static enum HeaderFormat {
    PREFERRED_BLOCK_SIZE(null, 48, 1),
    REPLICATION(PREFERRED_BLOCK_SIZE.BITS, 12, 1),
    STORAGE_POLICY_ID(REPLICATION.BITS, BlockStoragePolicySuite.ID_BIT_LENGTH,0);
  ...
  /**
   * 该文件包含的所有数据块,每一个BlockInfo是不同的数据块，BlockInfo中包含了备份信息
   */
  ...
  /** Is this file under construction? 文件是否正在构建中*/
  ...
  /* Start of Snapshot Feature 快照特性*/
   ...
   * 
   * @param includesLastUcBlock 是否包含最后一个正在构建的数据块
   *          If the last block is under construction, should it be included?
   * @param usePreferredBlockSize4LastUcBlock 如果第一个参数为true,该参数也为true,表示最后一个构建的数据块的大小为getPreferredBlockSize
   ...
   * @return file size
   * 计算该文件所有数据块的总和,即只是每一个数据块的单独一个备份
   */
