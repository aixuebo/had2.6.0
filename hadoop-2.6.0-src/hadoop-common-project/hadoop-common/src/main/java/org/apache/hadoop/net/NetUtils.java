...
public class NetUtils {
   ...
   * 创建一个socket,并且链接远程服务器
   * StandardSocketFactory为默认socket工厂
   */
   ...
   * 根据class去实例化SocketFactory,参数propValue即是class的全路径
   */
   ...
   * target如果没有scheme,则dummyscheme://+target,加dummyscheme的目的是能够正常解析host和port
   * 根据targe中的host和part组成ip对象
   */
  public static InetSocketAddress createSocketAddr(String target,
                                                   int defaultPort,
                                                   String configName) {
  ...
  public static InetSocketAddress createSocketAddrForHost(String host, int port) {
    ...
    try {
      InetAddress iaddr = SecurityUtil.getByName(resolveHost);//该方法不抛异常,说明该host是安全的host
      // if there is a static entry for the host, make the returned
      // address look like the original given host
      if (staticHost != null) {
        //host为域名,iaddr.getAddress()为ip地址
   ...
   * 
   * @param socket 对象
   * @param endpoint the remote address 链接的远程服务器地址
   * @param localAddr the local address to bind the socket to 绑定的本地地址
   * @param timeout timeout in milliseconds
   */
  public static void connect(Socket socket, 
  ...
  private static final Pattern ipPortPattern = // Pattern for matching ip[:port] ip地址+端口，或ip地址 不加端口都可以匹配上
  ...
  public static String getHostNameOfIP(String ipPort) {
    ...
    try {
      ...
      String ip = (-1 == colonIdx) ? ipPort
          : ipPort.substring(0, ipPort.indexOf(':'));//获取ip地址
   ...
   * @return hostname
   * 获取本地地址:Sc-201409041654/192.168.10.203
   */
