...
/dev/xvda1      20641404 12033060   7559820  62% /
tmpfs            4029028        0   4029028   0% /dev/shm
/dev/xvdb1     309633052 71546184 222358432  25% /data
 * 
 * */
...
public class DF extends Shell {
  ...
  /** Default DF refresh interval. 3秒默认时间*/
  public static final long DF_INTERVAL_DEFAULT = 3 * 1000;

  private final String dirPath;//目录的字符串路径
  private final File dirFile;//目录的文件对象
  private String filesystem;//参见类上面注释
  private String mount;//挂载点名称
  
  private ArrayList<String> output;//执行linux命令后的输出结果
  ...
  /** @return the capacity of the measured filesystem in bytes. 文件夹所在的磁盘总空间*/
  public long getCapacity() {
    return dirFile.getTotalSpace();
  }

  /** @return the total used space on the filesystem in bytes. 磁盘剩余空间*/
  public long getUsed() {
    return dirFile.getTotalSpace() - dirFile.getFreeSpace();
  }

  /** @return the usable space remaining on the filesystem in bytes. 磁盘可以空间*/
  public long getAvailable() {
    return dirFile.getUsableSpace();
  }

  /** @return the amount of the volume full, as a percent. 已经使用的磁盘占比*/
