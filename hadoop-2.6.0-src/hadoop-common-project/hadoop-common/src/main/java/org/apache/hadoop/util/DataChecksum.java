...
public class DataChecksum implements Checksum {
  ...
  public static enum Type {
    ...
    public final int id;
    public final int size;//校验和所占字节大小
   ...
   * 如果java虚拟机版本大于1.7,则生成PureJavaCrc32
   */
   ...
   */
    ...
    * If <i>reset</i> is true, then resets the checksum.
    * @return number of bytes written. Will be equal to getChecksumSize();
    * 将校验和int组装成字节数组存放到buf中,从offset中开始写入
    */
  ...
  private final int bytesPerChecksum;//每隔多少字节进行一次校验
  private int inSum = 0;//对多少个字节进行校验和了
  ...
  static public int getChecksumHeaderSize() {
    return 1 + SIZE_OF_INTEGER; // type byte, bytesPerChecksum int 一个字节的type,4个字节的int
  }
