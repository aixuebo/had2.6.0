...
public class AuthenticationFilterInitializer extends FilterInitializer {
  ...
  static final String SIGNATURE_SECRET_FILE = AuthenticationFilter.SIGNATURE_SECRET + ".file";//signature.secret.file
  ...
  public void initFilter(FilterContainer container, Configuration conf) {
    ...
    filterConfig.put(AuthenticationFilter.COOKIE_PATH, "/");//cookie.path

    //存储配置文件中以hadoop.http.authentication.开头的键值对
    ...
    try {
      ...
      reader.close();
      //存储key为signature.secret,value为文件的内容。
    ...
    //Resolve _HOST into bind address 读取bind.address对应的值
    String bindAddress = conf.get(HttpServer2.BIND_ADDRESS);
    //读取kerberos.principal对应的值
    ...
    if (principal != null) {
      ...
      //key为kerberos.principal,value为最终的principal值
