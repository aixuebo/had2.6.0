...
class NameCache<K> {
  ...
  private class UseCount {
    int count;//次数
    final K value;  // Internal value for the name 缓存的name值
  ...
  /** names used more than {@code useThreshold} is added to the cache 命中次数超过该值,则加入到cache中*/
  private final int useThreshold;

  /** of times a cache look up was successful 被缓存击中的命中次数 */
  private int lookups = 0;

  /** Cached names */
  final HashMap<K, K> cache = new HashMap<K, K>();

  /** Names and with number of occurrences tracked during initialization 未加入到cache前存储数据的地方*/
   ...
   */
  K put(final K name) {
    K internal = cache.get(name);//缓存命中则返回要提前的值
    if (internal != null) {
      lookups++;
      return internal;
    }
  ...
  /** Promote a frequently used name to the cache 将name添加到cache中.因为该name已经命中的次数超过了阀值 */
