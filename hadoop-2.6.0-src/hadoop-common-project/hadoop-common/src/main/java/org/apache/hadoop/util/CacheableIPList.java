...
public class CacheableIPList implements IPList {
  private final long cacheTimeout;//过期时间间隔
  private volatile long cacheExpiryTimeStamp;//下次过期时间
  ...
  public boolean isIn(String ipAddress) {
    ...
    if (cacheExpiryTimeStamp >= 0 && cacheExpiryTimeStamp < System.currentTimeMillis()) {//已经过期
      synchronized(this) {
